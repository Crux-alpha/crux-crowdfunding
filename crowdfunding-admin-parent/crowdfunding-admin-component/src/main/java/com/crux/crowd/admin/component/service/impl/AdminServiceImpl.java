...
import com.crux.crowd.common.util.ExceptionUtils;
...
public class AdminServiceImpl extends ServiceImpl<AdminMapper,Admin> implements AdminService{
	...
	public Admin login(String account, String password){
		...
		// 2、检查Admin是否为空
		if(admin == null) throw ExceptionUtils.loginFailed(CrowdConstant.TipsMessage.ACCOUNT_NOT_FOUNT);
		// 3、加密password
		...
		Optional.ofNullable(admin.getUserPswd()).filter(encoded::equalsIgnoreCase)
				.orElseThrow(ExceptionUtils::loginFailed);
		// 5、校验成功，返回Admin
		return admin;
	}
